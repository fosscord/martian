...
/**
* Represents the main Martian client
...
* @prop {Object} options Martian options
...
* @prop {Collection<Shard>} shards Collection of shards Martian is using
...
class Client extends EventEmitter {
    ...
    * @arg {Object} options Martian client options
    ...
    * @arg {Boolean} [options.autoreconnect=true] Have Martian autoreconnect when connection is lost
    ...
    * @arg {Object} [options.disableEvents] If disableEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](/Martian/docs/reference#ws-event-names)
    ...
    * @arg {Number} [options.latencyThreshold=30000] [DEPRECATED] The average request latency at which Martian will start emitting latency errors. This option has been moved under `options.rest`
    ...
    * @arg {Number | String} [options.maxShards=1] The total number of shards you want to run. If "auto" Martian will use Discord's recommended shard count.
    ...
    * @arg {Number} [options.rest.latencyThreshold=30000] The average request latency at which Martian will start emitting latency errors
    ...
    */
    getRESTChannel(channelID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("Martian REST mode is not enabled"));
        }
